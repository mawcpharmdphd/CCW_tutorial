...
load("/Users/yili/Desktop/Michael CCW project/Stored_data/wted_trt_0_30.Rdata")
load("/Users/yili/Desktop/Michael CCW project/Stored_data/zeros_0_30.Rdata")
...
start_30_recentstart_1_trt <- wted_trt_0_30 %>%
  filter(start_interval == 30, recentstart == 1) %>%
  mutate(treat = 1)
...
start_30_recentstart_1_zeros <- zeros_0_30 %>%
  filter(start_interval == 30, recentstart == 1) %>%
  mutate(
    treat = 0,
    Cumulative_IPCW = 1
...
interval_30 <- bind_rows(
  start_30_recentstart_1_trt,
  start_30_recentstart_1_zeros
  ) %>%
...
# Combine both
interval_30_mean_stddev_unwt <- bind_rows(by_treat, overall)
...
prep_tpose_interval_30_unwt <- interval_30_mean_stddev_unwt %>%
  mutate(
...
tposed_30_mean_unwt <- prep_tpose_interval_30_unwt %>%
...
tposed_30_stddev_unwt <- prep_tpose_interval_30_unwt %>%
...
# Now we can add flags to merge these sets together, adding the "analysis" flag for means and SDs.
tposed_30_mean_flag_unwt <- tposed_30_mean_unwt %>%
  mutate(
    analysis = "unwt"
  )

tposed_30_stddev_flag_unwt <- tposed_30_stddev_unwt %>%
  mutate(
    analysis = "unwt"
  )
...
merge_unwt_analysis <- tposed_30_mean_flag_unwt %>%
  left_join(tposed_30_stddev_flag_unwt, by = "analysis") %>%
  mutate(
    variable = "age",
...
# Renal overall
all_renalprop_unwt_30 <- interval_30 %>%
  count(renal) %>%
  mutate(PERCENT = 100 * n / sum(n))

# Sex by treat
sexprop_unwt_30 <- interval_30 %>%
  count(sex, treat) %>%
  group_by(treat) %>%
  mutate(PCT_COL = 100 * n / sum(n)) %>%
  ungroup()
...
# Sex overall
all_sexprop_unwt_30 <- interval_30 %>%
  count(sex) %>%
  mutate(PERCENT = 100 * n / sum(n))
...
only_Y_renal_unwt_30 <- renalprop_unwt_30 %>%
...
only_Y_all_renal_unwt_30 <- all_renalprop_unwt_30 %>%
  filter(renal == "Y") %>%
  mutate(
    variable = "ren",
...
only_1_sex_unwt_30 <- sexprop_unwt_30 %>%
...
only_1_all_sex_unwt_30 <- all_sexprop_unwt_30 %>%
  filter(sex == "1") %>%
  mutate(
    variable = "fem",
    PROP_all = PERCENT / 100
...
cat_var_30_unwt <- bind_rows(
  full_join(only_Y_renal_unwt_30, only_Y_all_renal_unwt_30, by = "variable"),
  full_join(only_1_sex_unwt_30, only_1_all_sex_unwt_30, by = "variable")
  ) %>%
  mutate(
...
interval_30_mean_stddev_wt <- interval_30 %>%
  group_by(treat) %>%
  summarise(
...
interval_30_mean_stddev_wt <- bind_rows(interval_30_mean_stddev_wt, overall_stats)
...
prep_tpose_interval_30_wt <- interval_30_mean_stddev_wt %>%
  mutate(treat = ifelse(is.na(treat), -999, treat))

# Next, we can transpose to put all the means and SDs into 1 observation.
tposed_30_mean_wt <- prep_tpose_interval_30_wt %>%
  select(treat, age_mean) %>%
  pivot_wider(names_from = treat, values_from = age_mean, names_prefix = "Mean_")

tposed_30_stddev_wt <- prep_tpose_interval_30_wt %>%
  select(treat, age_stddev) %>%
  pivot_wider(names_from = treat, values_from = age_stddev, names_prefix = "STDDEV_")

# Now we can add flags to merge datasets.
tposed_30_mean_flag_wt <- tposed_30_mean_wt %>%
  mutate(analysis = "wted") 

tposed_30_stddev_flag_wt <- tposed_30_stddev_wt %>%
  mutate(analysis = "wted") 

# Now we merge the datasets and calculate the SMD.
merge_wt_analysis <- full_join(
  tposed_30_mean_flag_wt, tposed_30_stddev_flag_wt, by = "analysis"
  ) %>%
  mutate(
    variable = "age",
    SMD = (Mean_1 - `Mean_-999`) / `STDDEV_-999`
...
# Renal overall
all_renalprop_wt_30 <- interval_30 %>%
  group_by(renal) %>%
  summarise(
...
# Sex by treat
sexprop_wt_30 <- interval_30 %>%
  group_by(sex, treat) %>% 
  filter(treat == 1) %>% # pick only treat = 1
  summarise(
    n = sum(Cumulative_IPCW, na.rm = TRUE),
    .groups = 'drop'
  ) %>%
...
# Next, we limit ourselves to one level of the variables (1 for female, Y for renal) and create flag for the variable name. 
# We also subset to treat = 1
only_Y_renal_wt_30 <- renalprop_wt_30 %>%
  filter(renal == "Y", treat == 1) %>%
  mutate(variable = "ren", PROP_1 = PCT_COL / 100)

only_Y_all_renal_wt_30 <- all_renalprop_wt_30 %>%
  filter(renal == "Y") %>%
  mutate(variable = "ren", PROP_all = PERCENT / 100)

only_1_sex_wt_30 <- sexprop_wt_30 %>%
  filter(sex == "1", treat == 1) %>%
  mutate(variable = "fem", PROP_1 = PCT_COL / 100)

only_1_all_sex_wt_30 <- all_sexprop_wt_30 %>%
  filter(sex == "1") %>%
  mutate(variable = "fem", PROP_all = PERCENT / 100)

# Next, we can combine datasets together and calculate SMDs.
cat_var_30_wt_renal <- only_Y_renal_wt_30 %>%
  full_join(only_Y_all_renal_wt_30, by = "variable") %>%
  mutate(
    SMD = (PROP_1 - PROP_all) / sqrt(((PROP_1 * (1 - PROP_1) + PROP_all * (1 - PROP_all)) / 2))
  ) 

cat_var_30_wt_fem <- only_1_sex_wt_30 %>%
  full_join(only_1_all_sex_wt_30, by = "variable") %>%
  mutate(
    SMD = (PROP_1 - PROP_all) / sqrt(((PROP_1 * (1 - PROP_1) + PROP_all * (1 - PROP_all)) / 2))
  ) 


cat_var_30_wt_renal$SMD
# -0.02688043
cat_var_30_wt_fem$SMD
# -0.001315262

# For Table 1, first column:
merge_unwt_analysis
all_sexprop_unwt_30
all_renalprop_unwt_30
