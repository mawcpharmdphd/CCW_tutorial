...
/*This code shows how to calculate these values for the 0-90 analysis/
...
PROC TRANSPOSE DATA=only_Y_renal_unwt_90 OUT=tposed_90_renal_unwt PREFIX=TREAT_;
	VAR PROP;
	ID treat;
	BY variable;
RUN;

PROC TRANSPOSE DATA=only_1_sex_unwt_90 OUT=tposed_90_sex_unwt PREFIX=TREAT_;
	VAR PROP;
	ID treat;
	BY variable;
RUN;
...
DATA cat_var_90_unwt;
	SET tposed_90_renal_unwt tposed_90_sex_unwt;
...
PROC TRANSPOSE DATA=only_Y_renal_wt_90 OUT=tposed_90_renal_wt PREFIX=TREAT_;
	VAR PROP;
	ID treat;
	BY variable;
RUN;

PROC TRANSPOSE DATA=only_1_sex_wt_90 OUT=tposed_90_sex_wt PREFIX=TREAT_;
	VAR PROP;
	ID treat;
	BY variable;
RUN;
...
DATA cat_var_90_wt;
	SET tposed_90_renal_wt tposed_90_sex_wt;
	SMD = (TREAT_1 - TREAT_0)/ SQRT( ( ( TREAT_1*(1-TREAT_1) + TREAT_0*(1-TREAT_0) ) / 2 ) );
RUN;
