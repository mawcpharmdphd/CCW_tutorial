...
/*This code shows how to calculate these values for the 0_90 analysis/
...
DATA start_90_recentstart_1_trt;
	SET wted_trt_0_90;
...
comparisons a bit easier. This has to be handled a bit differently in the "90_90" analysis where some of these people will have non-zero IPCW.*/

DATA start_90_recentstart_1_zeros;
	SET zeros_0_90;
	IF start_interval = 90 AND recentstart = 1;
	treat = 0;
	Cumulative_IPCW = 1;
...
DATA interval_90;
	SET start_90_recentstart_1_trt start_90_recentstart_1_zeros;
...
PROC MEANS DATA=interval_90 MEAN STDDEV; /*We want means and standard deviation*/
	CLASS treat; /*This calculates values in each group and overall*/
	VAR intv_age;
	OUTPUT OUT=interval_90_mean_stddev_unwt mean=age_mean stddev=age_stddev;
RUN;
...
DATA prep_tpose_interval_90_unwt;
	SET interval_90_mean_stddev_unwt;
	IF treat=. THEN treat = -999;
RUN;

/*Next, we can use PROC TRANSPOSE to put all the mean information into 1 observation*/

PROC TRANSPOSE DATA=prep_tpose_interval_90_unwt OUT=tposed_90_mean_unwt PREFIX=Mean_;
	VAR age_mean;
	ID treat;
RUN;
...
PROC TRANSPOSE DATA=prep_tpose_interval_90_unwt OUT=tposed_90_stddev_unwt PREFIX=STDDEV_;
	VAR age_stddev;
	ID treat;
RUN;
...
DATA tposed_90_mean_flag_unwt;
	SET tposed_90_mean_unwt;
...
DATA tposed_90_stddev_flag_unwt;
	SET tposed_90_stddev_unwt;
	analysis="unwt";
	DROP _name_;
RUN;
...
DATA merge_unwt_analysis;
	MERGE tposed_90_mean_flag_unwt tposed_90_stddev_flag_unwt;
	BY analysis;
	variable = "age";
...
PROC FREQ DATA=interval_90;
	TABLES renal*treat / OUT=renalprop_unwt_90 OUTPCT;
	TABLES renal / OUT=all_renalprop_unwt_90;
	TABLES sex*treat / OUT=sexprop_unwt_90 OUTPCT;
	TABLES sex / OUT=all_sexprop_unwt_90;
RUN;
...
DATA only_Y_renal_unwt_90;
	SET renalprop_unwt_90;
	IF renal = "Y";
	WHERE treat=1;
	variable = "ren";
	PROP_1=PCT_COL/100;
...
DATA only_Y_all_renal_unwt_90;
	SET all_renalprop_unwt_90;
	IF renal = "Y";
	variable = "ren";
	PROP_all=PERCENT/100;
RUN;

DATA only_1_sex_unwt_90;
	SET sexprop_unwt_90;
	IF sex="1";
	WHERE treat=1;
	variable = "fem";
...
DATA only_1_all_sex_unwt_90;
	SET all_sexprop_unwt_90;
	IF sex="1";
	variable = "fem";
	PROP_all=PERCENT/100;
RUN;
...
DATA cat_var_90_unwt;
	MERGE  only_Y_renal_unwt_90 only_Y_all_renal_unwt_90 only_1_sex_unwt_90 only_1_all_sex_unwt_90;
	BY variable;
...
PROC MEANS DATA=interval_90 MEAN STDDEV; /*We want means and standard deviation*/
	...
	OUTPUT OUT=interval_90_mean_stddev_wt mean=age_mean stddev=age_stddev;
RUN;
...
DATA prep_tpose_interval_90_wt;
	SET interval_90_mean_stddev_wt;
	IF treat=. THEN treat = -999;
RUN;

/*Next, we can use PROC TRANSPOSE to put all the mean information into 1 observation*/

PROC TRANSPOSE DATA=prep_tpose_interval_90_wt OUT=tposed_90_mean_wt PREFIX=Mean_;
	VAR age_mean;
	ID treat;
RUN;
...
PROC TRANSPOSE DATA=prep_tpose_interval_90_wt OUT=tposed_90_stddev_wt PREFIX=STDDEV_;
	VAR age_stddev;
	ID treat;
RUN;
...
DATA tposed_90_mean_flag_wt;
	SET tposed_90_mean_wt;
	analysis="wted"; /*The flag is set to unweighted, since these are the results before IPCW*/
	DROP _name_; /*We don't need this variable anymore*/
RUN;

DATA tposed_90_stddev_flag_wt;
	SET tposed_90_stddev_wt;
	analysis="wted";
	DROP _name_;
RUN;
...
DATA merge_wt_analysis;
	MERGE tposed_90_mean_flag_wt tposed_90_stddev_flag_wt;
	BY analysis;
	variable = "age";
	SMD = (mean_1 - mean__999) / STDDEV__999;
RUN;
...
PROC FREQ DATA=interval_90;
	TABLES renal*treat / OUT=renalprop_wt_90 OUTPCT;
	WEIGHT cumulative_IPCW;
	TABLES renal / OUT=all_renalprop_wt_90;
	WEIGHT cumulative_IPCW;
	TABLES sex*treat / OUT=sexprop_wt_90 OUTPCT;
	WEIGHT cumulative_IPCW;
	TABLES sex / OUT=all_sexprop_wt_90;
	WEIGHT cumulative_IPCW;
RUN;

/*Next, we limit ourselves to one level of the variables (1 for female, Y for renal) and create flag for the variable name. We also subset the normal tables to treat=1*/

DATA only_Y_renal_wt_90;
	SET renalprop_wt_90;
	IF renal = "Y";
	WHERE treat=1;
	variable = "ren";
...
DATA only_Y_all_renal_wt_90;
	SET all_renalprop_wt_90;
	IF renal = "Y";
	variable = "ren";
	PROP_all=PERCENT/100;
RUN;

DATA only_1_sex_wt_90;
	SET sexprop_wt_90;
	IF sex="1";
	WHERE treat=1;
	variable = "fem";
...
DATA only_1_all_sex_wt_90;
	SET all_sexprop_wt_90;
	IF sex="1";
	variable = "fem";
	PROP_all=PERCENT/100;
RUN;
...
DATA cat_var_90_wt;
	MERGE  only_Y_renal_wt_90 only_Y_all_renal_wt_90 only_1_sex_wt_90 only_1_all_sex_wt_90;
	BY variable;
	SMD = (PROP_1 - PROP_all)/ SQRT( ( ( PROP_1*(1-PROP_1) + PROP_all*(1-PROP_all) ) / 2 ) );
RUN;
